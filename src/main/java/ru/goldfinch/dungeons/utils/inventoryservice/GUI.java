...
import ru.goldfinch.dungeons.utils.inventoryservice.content.InventoryProvider;
...
public class GUI {
    ...
    private GUI(GUIManager manager) { this.manager = manager; }

    public Inventory open(Player player) { return open(player, 0); }
    public Inventory open(Player player, int page) {
        Optional<GUI> oldInv = this.manager.getInventory(player);
    ...
    public void close(Player player) {
        ...
        this.manager.setInventory(player, null);
        Bukkit.getScheduler().runTaskLater(Dungeons.getInstance(), player::closeInventory, 2L);
    ...
    public Optional<GUI> getParent() { return Optional.ofNullable(parent); }
    ...
    public static final class Builder {
        ...
        private List<GUIListener<? extends Event>> listeners = new ArrayList<>();
        ...
        public GUI build() {
            if(this.provider == null)
                throw new IllegalStateException("Ошибка! Отсутсвует параметр Provider");

            GUI inv = new GUI(Dungeons.getInstance().getGuiManager());
            inv.id = this.id;
